...
de_gene_ids = sapply(hb_de_list, pull, "Geneid")

direct_target_ids = mapply(get_direct_targets, hb_peak_list, hb_de_list, 
...
test = mapply("%in%", de_gene_ids, direct_target_ids)

de_gene_ids %>% unlist %>% unique %>% length

###To do generate df of common gene_ids with 0 and 1 depending if they are direct targets

test %>% sapply(sum)


brc1_dt = get_direct_targets(macs2_consensus_q0001, BRC1ind_data, ath_3kup_1kdown)


hb21_dt = get_direct_targets(hbind_results$filtered_tests)
...
BRC1_bound = find_overlaps(csaw_result_filtered,ath_3kup_1kdown) %>% as_tibble()
BRC1_bound_anno = ifelse(input$Geneid %in% BRC1_bound$gene_id, 1,0) %>% `names<-`(input$Geneid)

BRC1_bound_macs005 = find_overlaps(macs2_consensus_q005,ath_3kup_1kdown) %>% as_tibble()
BRC1_bound_anno_macs005 = ifelse(input$Geneid %in% BRC1_bound_macs005$gene_id, 1,0) %>% `names<-`(input$Geneid)

BRC1_bound_macs001 = find_overlaps(macs2_consensus_q001,ath_3kup_1kdown) %>% as_tibble()
BRC1_bound_anno_macs001 = ifelse(input$Geneid %in% BRC1_bound_macs001$gene_id, 1,0) %>% `names<-`(input$Geneid)

BRC1_bound_macs0001 = find_overlaps(macs2_consensus_q0001,ath_3kup_1kdown) %>% as_tibble()
BRC1_bound_anno_macs0001 = ifelse(input$Geneid %in% BRC1_bound_macs0001$gene_id, 1,0) %>% `names<-`(input$Geneid)

HB21_bound = find_overlaps(peak_list$HB21,ath_3kup_1kdown) %>% as_tibble()
HB21_bound_anno = ifelse(input$Geneid %in% HB21_bound$gene_id, 1,0) %>% `names<-`(input$Geneid)

HB40_bound = find_overlaps(peak_list$HB40,ath_3kup_1kdown) %>% as_tibble()
HB40_bound_anno = ifelse(input$Geneid %in% HB40_bound$gene_id, 1,0) %>% `names<-`(input$Geneid)

HB53_bound = find_overlaps(peak_list$HB53,ath_3kup_1kdown) %>% as_tibble()
HB53_bound_anno = ifelse(input$Geneid %in% HB53_bound$gene_id, 1,0) %>% `names<-`(input$Geneid)
