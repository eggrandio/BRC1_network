...
### TO DO: statistical test to check if BRC1ind up-regulated genes are enriched in BRC1 peaks
# https://statsandr.com/blog/chi-square-test-of-independence-in-r/
brc1_direct_targets
brc1_diff_exp = BRC1ind_data$Geneid

total_genes = data.frame("gene_id" = ath_genes$gene_id) %>% 
  mutate(dt = ifelse(gene_id %in% brc1_direct_targets, 1, 0),
         de = ifelse(gene_id %in% brc1_diff_exp, 1, 0))
