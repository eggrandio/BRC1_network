...
adjm = c(genes_by_grn$UP_C6, "AT3G18550") %>% data.frame(gene_id = .) %>% 
...
adjm3 = c(genes_by_grn$UP_C6, "AT3G18550") %>% data.frame(gene_id = .) %>%  
...
best_gene_symbols = left_join(biomart_gene_symbols, tair_db_gene_symbols, by = "tair_locus") %>% 
  mutate(tair_symbol = coalesce(tair_symbol, name_1)) %>% dplyr::select(c(tair_locus, tair_symbol)) %>% 
...
coex_data = get_coex_data(agi_ids)
coex_data = get_coex_data(agi_ids, use_biomart = TRUE)
N = 20819

coex_data_u = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-u.v21-01.G18957-S27427.combat_pca_subagging.ls.d/")
coex_data_r = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-r.v21-01.G18957-S14741.combat_pca_subagging.ls.d/")
N2 = 18957


# to check correlation between ATTED databases
coex_data = get_coex_data(agi_ids, as_matrix = FALSE)
coex_data_u = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-u.v21-01.G18957-S27427.combat_pca_subagging.ls.d/", as_matrix = FALSE)

common_genes = intersect(colnames(coex_data), colnames(coex_data_u))

mat_m = coex_data %>% as_tibble() %>% 
  dplyr::select(any_of(common_genes)) %>% 
  filter(gene_id %in% all_of(common_genes)) %>% 
  column_to_rownames("gene_id") %>% as.matrix()

mat_u = coex_data_u %>% as_tibble() %>% 
  dplyr::select(any_of(common_genes)) %>% 
  filter(gene_id %in% all_of(common_genes)) %>% 
  column_to_rownames("gene_id") %>% as.matrix()
