...
source("./R/difexp_analysis.R")
source("./R/volcano_plot.R")
source("./R/mds_ggplot.R")
source("./R/mode_value.R")
source("./R/dss2df.R")
...
exp_name = paste0(format(Sys.Date(),"%Y_%m_%d_"), "ChIP_GFPBRC1ind_")

# Create GRanges object with all arabidopsis genes from ensembl .gft file
...
write_gff3(macs2_consensus_q0001, file = "./output/macs2_q0001_consensus.gff")
...
write_gff3(macs2_consensus_q001, file = "./output/macs2_q001_consensus.gff")
...
write_gff3(macs2_consensus_q005, file = "./output/macs2_q005_consensus.gff")
...
names(csaw_result) = paste0("region", 1:sum(is.sig))
...
DGEL = estimateDisp(y, design)
fit = glmQLFit(y, design, robust=TRUE)
results = glmQLFTest(fit)

merged = mergeResults(data2, results$table, tol=200L)

is.sig = merged$combined$FDR <= 0.05
csaw_result = merged$regions[is.sig]
csaw_result$score = merged$combined$rep.logFC[is.sig]
csaw_result$FDR = merged$combined$FDR[is.sig]
names(csaw_result) = paste0("region", 1:sum(is.sig))
...
peak_files = list.files(path = "./output/", pattern = ".bed", all.files = TRUE,
           full.names = TRUE)

peak_files[5:7]
peak_file_names = sapply(strsplit(peak_files[5:7], "_"), function(x) x[2])

list1 = lapply(peak_files[5:7], read_bed) %>% GRangesList() %>% `names<-`(peak_file_names)

peak_file_names = c("csaw","csaw12")
list2 = lapply(peak_files[8:9], read_bed) %>% GRangesList() %>% `names<-`(peak_file_names)

list3 = c(list1,list2)

ext5 = 10000
ext3 = 1000
test = lapply(list3,normalizeToMatrix,
              target = tss,
              extend = c(ext5,ext3),
              verbose = FALSE,
              mean_mode = "w0", w = 25, 
...
top_anno = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = "grey30"),
                                                   axis_param = list(side = "left")))
                             
test2 = mapply(EnrichedHeatmap,test, column_title = names(test), 
               MoreArgs = list(col = c("white", "grey30"),
...
###
ht_list2 = sapply(test2,"+")

###
ht_list = NULL

for(i in seq_along(test2)){
  ht_list = ht_list + test2[[i]]
}
...
figure_filename = paste0("./output/peak_location_",ext5,"_",ext3,".png")
...
#### To do: plot HB Dapseq data relative to TSS
...
peak_list_200 = lapply(peak_list, function(x) filter(x, score>200))

ext5 = 3000
ext3 = 1000
test = lapply(peak_list_200,normalizeToMatrix,
              target = tss,
              extend = c(ext5,ext3),
              #value_column = 2, #use score as value for colors
              verbose = FALSE,
              mean_mode = "w0", w = 25, 
              background = 0, smooth = TRUE)
...
figure_filename = paste0("./output/HB_peak200_location_",ext5,"_",ext3,".png")
...
#### To do: select genes with peak in their 3u1d and plot peak signal vs RNAseq response in GFPBRC1ind (also abs(FC)?)
ath_3kup_1kdown
...
###
# heatmap
input = BRC1ind_data %>% select(c(Geneid,logFC_GFP_BRC1ind_vs_brc1)) %>% left_join(HBind_data,by="Geneid") %>% 
  left_join(aba_data_unfiltered) %>% 
  select(c(Geneid,logFC_GFP_BRC1ind_vs_brc1,logFC_brc1_ctr_vs_wt_ctr,logFC_hb21i_vs_gus,logFC_hb40i_vs_gus,logFC_hb53i_vs_gus)) 
...
###To do: add BRC1dep de microarray data

BRC1_bound = find_overlaps(csaw_result_filtered,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

BRC1_bound_anno = ifelse(input$Geneid %in% BRC1_bound$name, 1,0) %>% `names<-`(input$Geneid)

BRC1_bound_macs005 = find_overlaps(macs2_consensus_q005,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

BRC1_bound_anno_macs005 = ifelse(input$Geneid %in% BRC1_bound_macs005$name.y, 1,0) %>% `names<-`(input$Geneid)

BRC1_bound_macs001 = find_overlaps(macs2_consensus_q001,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

BRC1_bound_anno_macs001 = ifelse(input$Geneid %in% BRC1_bound_macs001$name.y, 1,0) %>% `names<-`(input$Geneid)


BRC1_bound_macs0001 = find_overlaps(macs2_consensus_q0001,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

BRC1_bound_anno_macs0001 = ifelse(input$Geneid %in% BRC1_bound_macs0001$name.y, 1,0) %>% `names<-`(input$Geneid)



HB21_bound = find_overlaps(peak_list$HB21,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

HB21_bound_anno = ifelse(input$Geneid %in% HB21_bound$name.y, 1,0) %>% `names<-`(input$Geneid)

HB40_bound = find_overlaps(peak_list$HB40,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

HB40_bound_anno = ifelse(input$Geneid %in% HB40_bound$name.y, 1,0) %>% `names<-`(input$Geneid)

HB53_bound = find_overlaps(peak_list$HB53,ath_3kup_1kdown) %>% `names<-`(paste0(names(.),"_",.$name)) %>% 
  as_tibble()

HB53_bound_anno = ifelse(input$Geneid %in% HB53_bound$name.y, 1,0) %>% `names<-`(input$Geneid)

anno_df = cbind(brc1_de_anno,
                brc1_array_anno,
                BRC1_bound_anno,
                BRC1_bound_anno_macs005,
                BRC1_bound_anno_macs001,
                BRC1_bound_anno_macs0001,
                HB21_bound_anno,
                HB40_bound_anno,
                HB53_bound_anno) %>% as.data.frame()
...
ha = HeatmapAnnotation(brc1_de_anno = brc1_de_anno,
                       brc1_array = brc1_array_anno,
                       BRC1bound = BRC1_bound_anno,
                       BRC1b2 = BRC1_bound_anno_macs005,
                       BRC1b3 = BRC1_bound_anno_macs001,
                       BRC1b4 = BRC1_bound_anno_macs0001,
                       HB21bound = HB21_bound_anno,
                       HB40bound = HB40_bound_anno,
                       HB53bound = HB53_bound_anno,
                       which = "row", 
