...
annotation_data = (BRC1ind_data$Geneid %in% brc1_direct_targets) %>% as.numeric()
...
### TO DO: hypergeometric test to check if BRC1ind up-regulated genes are statistically enriched in BRC1 peaks
...
test = data.frame(gene_id = filtered_data %>% rownames) %>% 
  mutate(HB21ind = case_when(
    gene_id %in% gene_list$`HB21ind up (1638)` ~ "up",
    gene_id %in% gene_list$`HB21ind down   (725)` ~ "dn",
    TRUE ~ "ns")) %>% 
  mutate(HB40ind = case_when(
    gene_id %in% gene_list$`HB40ind up   (567)` ~ "up",
    gene_id %in% gene_list$`HB40ind down   (161)` ~ "dn",
    TRUE ~ "ns")) %>% 
    mutate(HB53ind = case_when(
    gene_id %in% gene_list$`HB53ind up   (564)` ~ "up",
    gene_id %in% gene_list$`HB53ind down   (120)` ~ "dn",
    TRUE ~ "ns"))
