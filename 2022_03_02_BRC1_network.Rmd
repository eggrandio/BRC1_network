...
---
```{r Load required packages and functions, include=FALSE}
# Data manipulation
library(tidyverse)
library(data.table)
...
library(igraph)
library(RCy3) # Install Cytoscape and open it to send igraph networks to Cytoscape
...
peak_number = seq(810,820,1)
#peak_number = 813

for(n in peak_number){
n_consensus_peaks = lapply(peak_list, function(x, y) { helper_func(x,y) }, y = n) %>% 
  GRangesList() %>% get_consensus_peaks_GRangesList() %>% length
print(n_consensus_peaks)
}

# 813 peaks give 500 consensus peaks

consensus_peaks_500 = lapply(peak_list, function(x, y) { helper_func(x,y) }, y = 813) %>% 
  GRangesList() %>% get_consensus_peaks_GRangesList()

consensus_peaks_500 = consensus_peaks_500 %>% `seqlevelsStyle<-`("TAIR9")

test_fasta = getSeq(Athaliana, consensus_peaks_500) %>% `names<-`(consensus_peaks_500$peak_name)

writeXStringSet(test_fasta, "./output/test_BRC1_peaks.fa")


recentered_peaks = consensus_peaks_500 %>% anchor_center(.) %>% mutate(width = 200)

test_fasta2 = getSeq(Athaliana, recentered_peaks) %>% `names<-`(recentered_peaks$peak_name)

writeXStringSet(test_fasta2, "./output/test_BRC1_recentered_peaks.fa")





# convert to mapply and test for different y, the number of consensus peaks (REMEMBER TO SORT BY qVAL first!)

###

test = peak_list %>% lapply(as.data.frame)
...
```{r Find availabe data and tools to study ABA GRN}
# In the future, we might want to include available ChIP/DAP-seq data for other TFs and use inducible lines to generate RNA-seq data
...
# We will focus on DNA-binding transcription factors (GO:0003700).
# Retrieve description of transcriptional regulation related GOs (although for now we will only use GO:0003700)
tx_factor_gos = AnnotationDbi::select(GO.db, 
                                      keys=c("GO:0003700","GO:0140110","GO:0006355","GO:0010468"),
                                      columns=c("GOID","TERM","DEFINITION"),
                                      keytype="GOID")

# Retrieve GO annotation for ABA_GRN genes and filter DNA-binding transcription factors
...
# Check DAPseq/Chip-seq data availability of selected TFs and if there are vectors/lines from TRANSPLANTA consotrium
...
Ecker_ChIPseq = AnnotationDbi::select(org.At.tair.db, 
                                      keys = Ecker_ChIPseq_symbols,
                                      columns = c("SYMBOL", "TAIR"),
                                      keytype = "SYMBOL") %>% 
  dplyr::filter(!SYMBOL %in% c("DTAF1","DTAF2")) %>% 
  dplyr::filter(!(SYMBOL == "HB-7" & TAIR == "AT5G46880")) %>% 
  rename("gene_id" = TAIR)
...
# Import TRANSPLANTA vectors (Table S2 - TFs cloned in pER8GW vector) from Coego et al., 2014 - https://doi.org/10.1111/tpj.12443
transplanta_vectors = fread("./input/TRANSPLANTA_S2.txt", skip = 1) %>% 
  mutate(gene_id = gsub("\\..*","",`AGI code`) , 
         .before = `AGI code`)

# Import TRANSPANTA inducible lines from Coego et al., 2014 - https://doi.org/10.1111/tpj.12443
...
# Create data.frame to summarize available data/tools
ABA_GRN_TFs_available_data = ABA_GRN_CORE_TFs %>% 
  mutate(ChIPseq = ifelse((gene_id %in% Ecker_ChIPseq$gene_id), 1, 0),
         DAPseq = ifelse((gene_id %in% Ecker_DAPseq$gene_id), 1, 0),
         TRANSPLANTA_vectors = ifelse((gene_id %in% transplanta_vectors$gene_id), 1, 0)) %>% 
  left_join(transplanta_lines, by = "gene_id") %>% dplyr::rename("TRANSPLANTA_lines" = n) %>% 
  mutate(TRANSPLANTA_lines = ifelse(is.na(TRANSPLANTA_lines), 0, TRANSPLANTA_lines))

# As we still don't know which BRC1-dep GRN to use (or if we will use them all for the network analysis),
# Retrieve available data for all the BRC1-dep_in_FR TFs:
# BRC1_dependent_FR = fread("./input/BRC1dep_PC2013.txt") %>%
#   rename("gene_id" = Geneid) %>%
#   mutate(gene_id = toupper(gene_id)) %>%
#   mutate(direction = ifelse(logFC > 0, "up", "down"))
# BRC1_dep_FR_TFs = AnnotationDbi::select(org.At.tair.db,
#                                         keys = BRC1_dependent_FR$gene_id,
#                                         columns = c("TAIR", "GOALL"),
#                                         keytype = "TAIR") %>%
#   filter(GOALL %in% c("GO:0003700")) %>% dplyr::select(c("TAIR","GOALL")) %>% unique %>%
#   dplyr::rename("gene_id" = TAIR) %>% add_gene_annotation(gene_id_column = "gene_id", tidy_output = FALSE) %>%
#   dplyr::select(c("gene_id", "gene_symbol", "short_description"))
# 
# BRC1_dep_FR_TFs_available_data = BRC1_dep_FR_TFs %>%
#   mutate(ChIPseq = ifelse((gene_id %in% Ecker_ChIPseq$gene_id), 1, 0),
#          DAPseq = ifelse((gene_id %in% Ecker_DAPseq$gene_id), 1, 0),
#          TRANSPLANTA_vectors = ifelse((gene_id %in% transplanta_vectors$gene_id), 1, 0)) %>%
#   left_join(transplanta_lines, by = "gene_id") %>% dplyr::rename("TRANSPLANTA_lines" = n) %>%
#   mutate(TRANSPLANTA_lines = ifelse(is.na(TRANSPLANTA_lines), 0, TRANSPLANTA_lines))
# 
# # We will prioritize BRC1-dep_in_FR TFs that are BRC1/HBs direct targets (to check them first)
# BRC1_dep_FR_TFs_HBdt = BRC1_dep_FR_TFs_available_data %>% filter(gene_id %in% BRC1_direct_targets$gene_id |
...
```{r Draw BRC1-network}
# Define genes that are BRC1/HB direct targets and genes that are differentially expressed by BRC1/HB induction ####
# Define direct targets as genes that have a binding peak 3kb upstream and 1kb downstream of TSS (target_region)
...
# HB peaks (reanalyzed DAP-seq data from O'Malley et al., 2016)
peak_files = list.files(path = "./input/", pattern = "^DAP_ATHB\\d{2}_q1_es200_peaks.narrowPeak$", all.files = TRUE, full.names = TRUE)
peak_file_names = str_match(peak_files, "DAP_ATHB\\d{2}") %>% as.vector
peak_list = lapply(peak_files, read_narrowpeaks) %>% GRangesList() %>% `names<-`(peak_file_names)
...
# Generate lists of direct target and differentially expressed genes
# BRC1-related gene lists
BRC1ind_diffexp = brc1ind_results$filtered_tests %>% 
  dplyr::select(c(Geneid, logFC_GFP_BRC1ind_vs_brc1)) %>% 
  dplyr::rename("gene_id" = Geneid, "logFC" = logFC_GFP_BRC1ind_vs_brc1) %>% 
  mutate(direction = ifelse(logFC > 0, "up", "down")) %>% 
  data.table()
...
# Additional gene lists we might use in the analysis
# BRC1-dependent genes that respond to low R:FR in axillary buds from González-Grandío et al., 2013 - doi.org/10.1105/tpc.112.108480
BRC1_dependent_FR = fread("./input/BRC1dep_PC2013.txt") %>% 
  rename("gene_id" = Geneid) %>% 
  mutate(gene_id = toupper(gene_id)) %>% 
  mutate(direction = ifelse(logFC > 0, "up", "down"))

# Genes that are missregulated in brc1 axillary buds (our new data)
...
# ABA-responding genes from Nemhauser et al., 2006 - doi.org/10.1016/j.cell.2006.05.050
ABA_genes_nemhauser = fread("./input/ABA-responding_genes_Nemhauser2006.txt") %>% mutate(gene_id = toupper(gene_id))

# BRC1-dep ABA-responding genes from Gonzalez-Grandio et al., 2017 - doi.org/10.1073/pnas.1613199114
...
# BRC1-dep ABA-responding genes in axillary buds (our new data - (wt_aba vs wt_ctr - brc1_aba vs brc1_ctr))
...
# Check if HB_DTs are enriched in specific BRC1 GRNs ####
# Import GRNs defined in van Es et al., 2020
BRC1_grns = fread("./input/BRC1_GRNs_vanEs2020.txt")
grns = BRC1_grns %>% pull(cluster) %>% unique
# make lists of genes for each GRN
genes_by_grn = sapply(grns, 
                      function(x) dplyr::filter(BRC1_grns, cluster == x & relevance == "core") %>% pull(gene_id),
                      simplify = FALSE, 
                      USE.NAMES = TRUE)

# Generate data for Chi-square test of HB_DTs enrichment
# We want to test if a gene being in a GRN (variable 1) is independent of it being HB direct target (variable 2)
# Generate list of total genes
...
# Generate list of HB direct target (direct target of ANY HB)
# HB_dt_up = Reduce(union, list(HB21_direct_targets %>% filter(direction == "up") %>% pull(gene_id),
#                               HB40_direct_targets %>% filter(direction == "up") %>% pull(gene_id),
#                               HB53_direct_targets %>% filter(direction == "up") %>% pull(gene_id)))

HB_dt = Reduce(union, list(HB21_direct_targets %>% pull(gene_id),
                           HB40_direct_targets %>% pull(gene_id),
                           HB53_direct_targets %>% pull(gene_id)))

# Data for variable 2 (chance of a gene being a HB direct target)
# Create it as a factor to assign level order to make sure chi-square generated tables are in the same order 
# (important to extract expected and observed values for final output)
chi_input_2 = factor(ifelse(total_genes %in% HB_dt, "dt", "not_dt"), levels = c("dt", "not_dt"))

# Data for variable 1 (chance of a gene being in a GRN)
chi_input_1 = sapply(genes_by_grn, 
                     function(x) factor(ifelse(total_genes %in% x, "in_cluster", "not_in"), levels = c("in_cluster", "not_in")),
                     simplify = FALSE, 
                     USE.NAMES = TRUE)

# Create table for each GRN for chi-square test
table_list = lapply(chi_input_1, function(x) table(chi_input_2, x))

# Make chi-square test for each GRN
test_list = lapply(table_list, chisq.test)

# Generate a "nice" output from the chi-square test list
...
significance = symnum(p_value, 
                      ...
                      symbols = c("***", "**", "*", ".", " ")) %>% as.vector()
# Create final output
test_result = data.frame(id = names(test_list),
                         expected = round(expected,0),
                         observed,
                         p_value, 
                         significance,
                         row.names = NULL) %>% 
  dplyr::mutate(direction = case_when(observed > expected ~ "enriched",
                                      observed < expected ~ "depleted",
                                      observed == expected ~ "same"),
                .after = observed) %>% 
  dplyr::mutate(fold_over_expected = round(observed/expected, 2),
                .after = observed)

test_result

# Do it also using BRC1_dep_FR genes as "total" (it will be less significant compared to total genes, but cluster 6 is still the most enriched)
#total_genes = BRC1_dependent_FR$gene_id

# Build BRC1-network ####
# Check igraph tutorial (https://kateto.net/networks-r-igraph)
# Use 48 genes from Cluster UP6 (+BRC1) from van Es et al., 2020
input_genes = c(genes_by_grn$UP_C6, "AT3G18550")

# Or with BRC1-dep ABA responding genes in buds (our new data)
# BRC1_HBs = c("AT3G18550",
#              "AT2G18550",
#              "AT4G36740",
#              "AT5G66700")
# input_genes = c(BRC1_dep_ABA_responding$gene_id, BRC1_HBs) %>% unique

# random_genes = sample(total_genes, length(c(BRC1_dep_ABA_responding$gene_id, BRC1_HBs)))
# 
# input_genes = c(random_genes, BRC1_HBs)

# Create adjacency matrices to generate graph using igraph
adjm = input_genes %>% data.frame(gene_id = .) %>% 
...
# Adjacency matrix has to be square. Also, we transpose it so igraph interprets directionality of interactions in the correct direction
adjm2 = expand_matrix(adjm) %>% t()

# Make graph from adjacency matrix so it stores node and edge information (node = genes, edge = interactions)
g1 = graph_from_adjacency_matrix(adjm2)

# Create adjacency matrix for differential expression the same way
adjm3 = input_genes %>% data.frame(gene_id = .) %>%  
...
# We remove loops as we are using inducible lines, so BRC1/HBs will be differentially expressed
g2b = simplify(g2, 
               remove.loops = T) 

# Merge two adjacency matrices to have both types of interactions (direct and differentially expressed)
# Probably there is a more elegant way of joining both interactions, but this works for now...
# Assign edge type to each graph
E(g1)$type = "direct_target"
E(g2)$type = "diff_exp"
#edge_attr(g1)
#vertex_attr(g1)

# Merge two graphs and make a new edge type merging both edge types
g3 = igraph::union(g1, g2)

t1 = edge_attr(g3)$type_1
t2 = edge_attr(g3)$type_2
t1[is.na(t1)] <- t2[which(is.na(t1))]
...
####
# TO DO: 
# tidy retrieval of best gene symbol
# read ATTED-II mRNAseq coexpression data and check if there is data for BRC1 and HB21 (there is no data for them in the Ath-U ATTED database)
# finish polishing network code
# create network attributes from R using RCy3 (or change them manually with cytoscape)

# Change node lable from agi id to gene symbol (it is tricky to select the best gene symbol name...)
# BiomaRt seems to return better names, so we start from those, 
# if any symbol is missing, we add it from org.At.tair.db and if no symbol, we add agi code (and we fix some of them manually in the end)
agi_ids = V(g3b)$name

biomart_gene_symbols = getBM(attributes = c("tair_locus", "tair_symbol"),
                             filters = "tair_locus",
                             values = agi_ids,
                             mart = ensembl) %>% 
  group_by(tair_locus) %>% dplyr::slice(1) %>% ungroup %>% 
  mutate(tair_symbol = replace(tair_symbol, tair_symbol == "", NA))

tair_db_gene_symbols = AnnotationDbi::select(org.At.tair.db,
                                             keys = agi_ids,
                                             columns = c("TAIR", "SYMBOL"),
                                             keytype = "TAIR") %>% 
  group_by(TAIR) %>% dplyr::mutate(name_n = paste0("name_", 1:n())) %>% 
  pivot_wider(id_cols = TAIR, values_from = SYMBOL, names_from = name_n) %>% 
  dplyr::rename(tair_locus = TAIR)

best_gene_symbols = data.frame(tair_locus = agi_ids) %>% 
  left_join(.,biomart_gene_symbols, by = "tair_locus") %>% 
  left_join(., tair_db_gene_symbols, by = "tair_locus") %>% 
  mutate(tair_symbol = coalesce(tair_symbol, name_1)) %>% 
  dplyr::select(c(tair_locus, tair_symbol)) %>% 
  mutate(tair_symbol = case_when(tair_locus == "AT2G18550" ~ "HB21",
                                 tair_locus == "AT4G36740" ~ "HB40",
                                 tair_locus == "AT1G77450" ~ "NAC032",
                                 TRUE ~ tair_symbol)) %>% 
  mutate(tair_symbol = str_remove(tair_symbol, "^[aA][tT]")) %>% 
  mutate(tair_symbol = coalesce(tair_symbol, tair_locus))

V(g3b)$label = best_gene_symbols %>% pull(tair_symbol)

# To calculate hierarcy of GRN, use in-degree and out-degree (from: https://doi.org/10.1038/nature11245)
#deg_total = igraph::degree(graph_input)
graph_input = g3b
deg_out = igraph::degree(graph_input, mode = "out")
deg_in = igraph::degree(graph_input, mode = "in")

hierarchy = ((deg_out - deg_in)/(deg_out + deg_in)) 

hierarchy %>% sort(decreasing = TRUE)%>% plot()

degree_distribution(g1, cumulative = TRUE) %>% plot()
degree_distribution(g2, cumulative = TRUE) %>% plot()
degree_distribution(g3b, cumulative = TRUE) %>% plot()

# Add hierarchy information to nodes (not relevant right now, but might be useful when we incorporate additional DAPseq/RNAseq data)
V(g3b)$hierarchy = hierarchy

# Send network to Cytoscape to edit "manually"
library(RCy3)
createNetworkFromIgraph(g3b)
# edge_attr(g3b)
# style.name = "dataStyle"
# createVisualStyle(style.name)
copyVisualStyle("Ripple", "custom_style")
setVisualStyle("custom_style")
...
setEdgeTargetArrowColorMapping(table.column = "type_3",
                    table.column.values = c("diff_exp","direct_target"), 
                    colors = c("#666666", "#D63E3E"), 
                    style.name = "custom_style", 
                    mapping.type = "d")

setEdgeColorMapping(table.column = "type_3",
                    table.column.values = c("diff_exp","direct_target"), 
                    colors = c("#666666", "#D63E3E"), 
                    style.name = "custom_style", 
                    mapping.type = "d")
...
```{r In progress: check clustering by coregulation with ATTED-II data}
# Read files downloaded from ATTED-II to retrieve coexpresion data ( https://atted.jp/download/ )
# r is rna-seq data, m is microarray data and u is union of both (see paper methods - doi.org/10.1093/pcp/pcac041)
agi_ids = BRC1_dependent_FR %>% filter(direction == "up") %>% pull(gene_id)

coex_data_marray = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-m.v21-01.G20819-S12686.combat_pca_subagging.ls.d/")
N = 20819

coex_data_union = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-u.v21-01.G18957-S27427.combat_pca_subagging.ls.d/")
coex_data_rnaseq = get_coex_data(agi_ids, data_dir = "D:/ATTED/Ath-r.v21-01.G18957-S14741.combat_pca_subagging.ls.d/")
N2 = 18957

### TO DO CHECK HOW TO MERGE BOTH DATASETS

# to check correlation between ATTED databases
common_genes = intersect(colnames(coex_data_marray), colnames(coex_data_rnaseq))

ggVennDiagram(list(BRC1dep = agi_ids, 
                   RNAseq = colnames(coex_data_rnaseq), 
                   MArray = colnames(coex_data_marray)))

Reduce(setdiff, list(agi_ids, colnames(coex_data_rnaseq), colnames(coex_data_marray)))

### diff_genes = setdiff(colnames(coex_data), colnames(coex_data_u))

mat_m = coex_data_marray %>% as.data.frame %>% 
  rownames_to_column("gene_id") %>% 
  dplyr::select(c(gene_id, any_of(common_genes))) %>% 
  filter(gene_id %in% all_of(common_genes)) %>% 
  column_to_rownames("gene_id") %>% as.matrix()

mat_u = coex_data_union %>% as.data.frame %>% 
  rownames_to_column("gene_id") %>% 
  dplyr::select(c(gene_id, any_of(common_genes))) %>% 
  filter(gene_id %in% all_of(common_genes)) %>% 
  column_to_rownames("gene_id") %>% as.matrix()

correlation_by_column = sapply(1:ncol(mat_m), function(i) cor(mat_m[,i], mat_u[,i], use="complete.obs"))
  
plot(correlation_by_column %>% sort,
     ylim=c(0, 1))


### TO DO: cluster genes using union dataset and microarray dataset (maybe generate a new-union dataset following ATTED paper methods)
### Compare genes in each cluster (maybe do it first only with common genes to compare reliability of clustering)
...
# For LS
d <- read.delim("./input/BRC1_dep_UP_loclocLS.txt") %>% column_to_rownames("X")
N <- 18957
d.dist <- as.dist(log2(N) - d)
d.hc <- hclust(d.dist, method="complete")
#plot (d.hc, hang=-1)
...
clusters = cutree(d.hc, cluster_number,order_clusters_as_data=T) %>% 
  as.factor() %>% fct_inorder() %>% fct_rev()

dd = d.hc %>% as.dendrogram(hang = -1) %>%
  set("labels_cex", 0.6) %>% 
