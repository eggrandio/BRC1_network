...
genes_by_grn = sapply(grns, 
                      function(x,y) dplyr::filter(y, cluster == x & relevance == "core") %>% pull(gene_id), 
                      y = BRC1_grns,
                      simplify = FALSE, 
                      USE.NAMES = TRUE)
...
chi_input_2 = factor(ifelse(total_genes %in% HB_dt_up, "dt", "not_dt"), levels = c("dt", "not_dt"))

# Data for variable 1 (chance of a gene being in a GRN)
chi_input_1 = sapply(genes_by_grn, 
                     function(x,y) factor(ifelse(y %in% x, "in_cluster", "not_in"), levels = c("in_cluster", "not_in")),
                     y = total_genes,
                     simplify = FALSE, 
                     USE.NAMES = TRUE)
...
# BiomaRt seems to return 
agi_ids = V(g3b)$name
gene_symbols = mapIds(org.At.tair.db, keys = agi_ids, column = "SYMBOL", keytype = "TAIR")
gene_symbols[which(is.na(gene_symbols))] <- names(gene_symbols)[which(is.na(gene_symbols))]

test2 = AnnotationDbi::select(org.At.tair.db,
                              keys = agi_ids,
                              columns = c("TAIR", "SYMBOL"),
                              keytype = "TAIR")

test2b = getBM(attributes = c('tair_locus', 'tair_symbol'),
               filters = 'tair_locus',
               values = agi_ids,
               mart = ensembl)
test2b

#TO DO: Select the "best" gene names (e.g., BRC1 instead of AtBRC1)
test3 = test2 %>% mutate(SYMBOL = coalesce(SYMBOL,TAIR)) %>% group_by(TAIR) %>% 
  dplyr::mutate(name_n = paste0("name_", 1:n())) %>% 
  pivot_wider(id_cols = TAIR, values_from = SYMBOL, names_from = name_n)

test_y = c(1,2,3,1,1,1,1,1,3,1,1,1,1,1,2,1,2,1,3,1,1,1,2,1,3,1,3) + 1

i = 1
best_gene_names = character()
for(j in test_y) {
  #print(as.character(test3[i,j]))
  best_gene_names[[i]] = as.character(test3[i,j])
  i = i + 1
}

V(g3b)$label = best_gene_names
